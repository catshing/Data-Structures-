...
public class BinarySearchTree<T> 
{
  ...
  protected Comparator<T> comp;   // used for all comparisons, this is an object of a class ( that can allow us to do comparisons of nodes) 
  ...
  // Creates an empty BST object - uses the natural order of elements.
  {
    ...
    {
       public int compare(T element1, T element2) // inner classes, defining the class right here inside another class
       {
         return ((Comparable)element1).compareTo(element2);// the comparator class, compare 2 elements, return -0 or postive 
       }
    };
  }
  ...
  // otherwise returns the smallest element of the tree.
  {
    ...
    else
    {
       ...
       while (node.getLeft() != null)
         node = node.getLeft(); // all the way down to the left
       return node.getInfo();
    }
  }
  ...
  //number of nodes with 1 child, traverse the tree and find the nodes that have only 1 child
  public T max()
  // If this BST is empty, returns null;
