...
public class WeightedGraph<T> implements WeightedGraphInterface<T>
{
	...
	{
		if (indexIs(vertex) >= 0 ) return true;
		return false;
	}
	...
	// Returns the index of vertex in vertices.
	{
		for (int index=0; index<vertices.length; index++) {
			if ( vertex.equals(vertices[index])) {return index;}
		}
		return -1;
	}
	...
	{
		for (T tmp:this.vertices) {
			if (tmp == null)
				break;
			Intersection in = (Intersection) tmp;
			in.unMark();
		}
	}
	...
	// Marks vertex.	
	{
		Intersection in = (Intersection) vertex;
		in.mark();
	}
	...
	{
		Intersection i = (Intersection) vertex;
		return i.isMarked();
	}
