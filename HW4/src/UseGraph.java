...
{
	public static int shortestPaths(WeightedGraphInterface<Intersection> graph,
	...
	{
		Path path;
		Path savePath;         // for saving on priority queue
		int minDistance;
		int newDistance;

		PriQueueInterface<Path> pq = new HeapPriQ<Path>(50);   // Assume at most 50 vertices
		Intersection vertex;
		QueueInterface<Intersection> vertexQueue = new LinkedQueue<Intersection>();

		graph.clearMarks();
		savePath = new Path(startVertex, startVertex, 0);
		pq.enqueue(savePath);
...
//		System.out.println("------------------------------------");
		...
		{
			path = pq.dequeue();
			if (!graph.isMarked(path.getToVertex()))
			{
				vertex = path.getToVertex();
...
//				System.out.println(path);
				...
				vertexQueue = graph.getToVertices(path.getFromVertex());
				while (!vertexQueue.isEmpty())
				{
					...
					if (!graph.isMarked(vertex))
					{
						newDistance = minDistance 
								+ graph.weightIs(path.getFromVertex(), vertex);
						savePath = new Path(path.getFromVertex(), vertex, newDistance);
						pq.enqueue(savePath);
					}
				}
			}
